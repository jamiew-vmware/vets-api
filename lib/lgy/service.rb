...
module LGY
  class Service < Common::Client::Base
    ...
    def coe_status
      ...
      if determination_status == 'ELIGIBLE' && app_status == 404
        { status: 'ELIGIBLE', reference_number: determination_ref_num }
      elsif determination_status == 'UNABLE_TO_DETERMINE_AUTOMATICALLY' && app_status == 404
        { status: 'UNABLE_TO_DETERMINE_AUTOMATICALLY', reference_number: determination_ref_num }
      elsif determination_status == 'ELIGIBLE' && app_status == 200
        { status: 'AVAILABLE', application_create_date: app_create_date, reference_number: determination_ref_num }
      elsif determination_status == 'NOT_ELIGIBLE'
        { status: 'DENIED', application_create_date: get_determination.body['determination_date'],
          reference_number: determination_ref_num }
      elsif determination_status == 'PENDING' && app_status == 404
      ...
      elsif determination_status == 'PENDING' && get_application.body['status'] == 'SUBMITTED'
        # SUBMITTED & RECEIVED ARE COMBINED ON LGY SIDE
        { status: 'PENDING', application_create_date: app_create_date, reference_number: determination_ref_num }
      elsif determination_status == 'PENDING' && get_application.body['status'] == 'RETURNED'
        { status: 'PENDING_UPLOAD', application_create_date: app_create_date, reference_number: determination_ref_num }
      end
    end
